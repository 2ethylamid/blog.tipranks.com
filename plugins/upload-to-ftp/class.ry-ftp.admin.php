...
class RY_FTP_admin {
	...
	public static function init() {
		if( !self::$initiated ) {
			...
			if( !empty($currentLocale) ) {
				$moFile = RY_FTP_PLUGIN_DIR . '/lang/' . $currentLocale . '.mo';
	...
	public static function plugin_action_links($links, $file) {
		if( $file == RY_FTP_PLUGIN_BASENAME ) {
			$links[] = '<a href="options-general.php?page=upload-to-ftp&tab=ftp">' . __('Settings', 'upload-to-ftp') . '</a>';
		}
		return $links;
	}

	public static function admin_menu() {
		add_submenu_page('options-general.php', 'Upload to FTP', __('Upload to FTP', 'upload-to-ftp'), 'manage_options', 'upload-to-ftp', array(__CLASS__, 'setting_page'));
	}
	...
	protected static function save_ftp_setting() {
		...
		if( !$ftp_resource ) {
			$info .= '<p>' . __('FTP connect error', 'upload-to-ftp') . ' <strong>' . $ftp_host . '</strong> : <strong>' . $ftp_port . '</strong></p>';
		} else {
			if( @!ftp_login($ftp_resource , $ftp_username, $ftp_password) ) {
				$info .= '<p>' . __('FTP login error with username', 'upload-to-ftp') . ' <strong>' . $ftp_username . '</strong></p>';
			} else {
				ftp_pasv($ftp_resource, (bool) $ftp_mode);
				if( @!ftp_chdir($ftp_resource, $ftp_dir) ) {
					$info .= '<p>' . __('FTP open directory failure', 'upload-to-ftp') . ' <strong>' . $ftp_dir . '</strong></p>';
				} else {
					...
					if( @!ftp_put($ftp_resource, $ftp_dir . $test_file_name, $test_file_path, FTP_BINARY) ) {
						$info .= '<p>' . __('FTP is not writable', 'upload-to-ftp') . ' <strong>' . $ftp_dir . '</strong></p>';
					} else {
						$ftp_uplode_ok = true;
						$body = '';
						if( ini_get('allow_url_fopen') ) {
							$body = @file_get_contents($html_link_url . $test_file_name);
						} elseif ( function_exists('curl_init') ) {
							$ch = curl_init();
							curl_setopt($ch, CURLOPT_URL, $html_link_url . $test_file_name);
						...
						if( $body != 'abcdefghijklmnopqrstuvwxyz' ) {
							$info .= '<p>' . __('HTML link url don\'t match FTP dir', 'upload-to-ftp') . '</p>';
							$info .= '<br />' . $html_link_url . $test_file_name;
						} else {
		...
		if( empty($info) ) {
			$info = '<div id="message" class="updated"><p>' . __('Updated FTP Options Success', 'upload-to-ftp') . '</p></div>';
	...
	protected static function save_setting() {
		...
		RY_FTP::update_option('options', RY_FTP::$options);
		$info = '<div id="message" class="updated"><p>' . __('Updated Basic Options Success', 'upload-to-ftp') . '</p></div>';
		echo $info;
	}
